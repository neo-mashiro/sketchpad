/* 
   ...
   units, the translation distance is always absolute, not affected by any scaling factor.
   translation can be local or global, that is, relative to the model space or the world
   space, this is controlled by the `Space` parameter.
   ...
   Rotation:
   --------------------------------------------------------------------------------------
   rotation can be either local or global, just like translation. Our rotate function
   expects all angle to be measured in degrees. For interpolation between rotations, see
   the `Slerp()` function in `utils::math` namespace.
